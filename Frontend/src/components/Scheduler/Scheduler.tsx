...
import { BookingsFilter, GetBookingResponse } from '@/services/BookingService';
...
export default function Scheduler({
  ...
  const events = useMemo(() => {
    return bookings.map((b) => ({
      id: b.id,
      title: b.title,
      start: dayjs(b.startOfBookingTime).format('YYYY-MM-DD HH:mm:ss'),
      end: dayjs(b.endOfBookingTime).format('YYYY-MM-DD HH:mm:ss'),
  ...
  const slotWidth = containerWidth
    ? Math.max(Math.floor((containerWidth - resourceLabelWidth) / numberOfHours), 40)
    : 150;
  ...
  return (
    <div className="scheduler" ref={containerRef}>
        ...
        renderResourceLabel={(resource) => (
          <div className="scheduler__channel">
            ...
            <div className="scheduler__channel-info">
              ...
              <span className="scheduler__channel-capacity">{String(resource.payload?.capacity)} мест</span>
            </div>
          </div>
        )}
      ...
      <div className="scheduler__legend">
        <Badge color="blue" variant="light">Ваша бронь</Badge>
        <Badge color="orange" variant="light">Забронировано</Badge>
      </div>
    </div>
  );
}